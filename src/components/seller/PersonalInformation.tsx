  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/${user._id}/sellerdraft?draft=personalInfo`,
        values,
        {
          headers: {
      ...
      console.log(response.data.data, "personal info response");
      ...
      <div className="flex-1 space-y-4 p-8 pt-6">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <Button
              type="submit"
              className="w-full mt-4 bg-blue-600 text-white rounded-md"
            >
              Submit
            </Button>
          </form>
